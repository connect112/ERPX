 ...
 *
 * This file is byte-identical to apps/web/src/api/client.ts and
 * apps/student-portal/src/api/client.ts (confirmed by direct diff), so
 * this test suite mirrors their already-proven coverage rather than
 * reinventing it — trainer-portal is nonetheless a separately built and
 * shipped app with its own bundle, so it needs its own real test run, not
 * just a reference to the other two apps'.
 *
