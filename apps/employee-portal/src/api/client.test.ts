 ...
 *
 * This file is byte-identical to apps/web/src/api/client.ts (inherited
 * unmodified from apps/student-portal, itself confirmed byte-identical
 * during that app's production-readiness audit), so this test suite
 * mirrors apps/web/src/api/client.test.ts's proven coverage rather than
 * reinventing it — employee-portal is nonetheless a separately built and
