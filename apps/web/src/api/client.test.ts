 ...
 *
 * This is the single most safety-critical piece of untested frontend
 * logic identified in the production-readiness audit: every authenticated
 * request in the app goes through it, and a bug here (e.g. retrying
 * forever, leaking a stale token, or firing N refresh calls for N
 * concurrent 401s) would be a real, user-visible auth failure.
 *
 * Only the HTTP layer is mocked (via axios-mock-adapter, attached to the
 * real `apiClient` instance's adapter) — the interceptors themselves and
 * the real Zustand auth store both run unmocked, so these tests exercise
 * the actual production code path, not a stand-in for it.
 */
