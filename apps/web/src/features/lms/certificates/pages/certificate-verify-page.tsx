...
import type { CertificateVerificationResponse } from "@/features/lms/certificates/api/certificates-api";
...
export function CertificateVerifyPage() {
  ...
  const onVerify = async () => {
    ...
    setResult(null);
    try {
      const { data } = await apiClient.get<CertificateVerificationResponse>(
        `/lms/certificates/verify/${encodeURIComponent(certificateNumber.trim())}`
      );
      setResult(data);
    } catch {
          ...
          {result && result.valid && (
            <div className="flex items-start gap-3 rounded-md border border-emerald-500/30 bg-emerald-500/10 p-4">
              ...
              <div className="text-sm">
                ...
                {result.course_title && <p>Course: {result.course_title}</p>}
                {result.issued_at && (
