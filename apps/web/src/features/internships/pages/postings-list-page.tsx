...
import { usePostingsList } from "@/features/internships/api/internships-hooks";
...
export function PostingsListPage() {
  ...
  const [status, setStatus] = useState<InternshipPostingStatus | "all">("all");
  ...
  return (
    <div className="space-y-6 p-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Internship Postings</h1>
          <p className="mt-1 text-muted-foreground">
            Internship openings that students can browse and apply to.
          </p>
        </div>
      ...
      <Card>
        <CardContent className="space-y-4 p-6">
          <Select
            value={status}
            onValueChange={(value) => {
              setStatus(value as InternshipPostingStatus | "all");
              setSkip(0);
            }}
          >
          ...
          {isError && (
            <p className="py-8 text-center text-sm text-destructive">
              Failed to load internship postings. Please try again.
            </p>
          )}

          {!isLoading && !isError && (data?.items.length ?? 0) === 0 && (
            <p className="py-8 text-center text-sm text-muted-foreground">
              No internship postings found. Create your first posting to get started.
            </p>
          )}
              ...
              <TableBody>
                {data?.items.map((posting) => (
                  <TableRow
                    ...
                    onClick={() => navigate(`/internships/postings/${posting.id}`)}
                  >
                    ...
                    <TableCell className="text-muted-foreground">
                      {posting.stipend != null ? posting.stipend.toLocaleString() : "—"}
                    </TableCell>
                    <TableCell>
