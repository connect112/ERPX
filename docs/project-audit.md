...
| Marketing overview landing-page-list materialization removed (finding #30, 2026-07-30) — `marketing_overview` fetched up to 10k pages (list count+select) only to feed a page-id `IN` view aggregate; replaced with one org-scoped `count(*) ... JOIN landing_pages WHERE organization_id` join. Profiled page section 3→1 query, overview 8→6 statements, peak memory −44%; values identical (equivalence test). `campaign_performance` unchanged | `modules/marketing/landing_pages/repository.py`, `modules/marketing/analytics/service.py` |
| Marketing overview referral materialization removed (finding #31, 2026-07-30) — `marketing_overview` loaded up to 10k `Referral` objects for a count + two Python status loops; replaced with one `SELECT status, count(*) ... GROUP BY status`. Profiled referrals section 2→1 query, peak memory −65% (1292→457 KiB); figures identical; fail-before/pass-after test | `modules/marketing/referrals/repository.py`, `modules/marketing/analytics/service.py` |
| Marketing overview coupon-list materialization removed (finding #29, 2026-07-30) — `marketing_overview` fetched up to 10k coupons (list count+select) only to feed a 10k-id `IN` aggregate; replaced with one org-scoped `count/sum ... JOIN coupons WHERE organization_id` join. Profiled coupon section 3→1 query, overview 10→8 statements, peak memory −22%; values identical (equivalence test). `campaign_performance` unchanged | `modules/marketing/coupons/repository.py`, `modules/marketing/analytics/service.py` |
| Marketing overview per-campaign lead-count N+1 fixed (finding #28, 2026-07-30) — `marketing_overview` looped campaigns calling `list_for_organization(limit=1)` (2 queries/campaign) to sum lead counts; replaced with one `count(*) ... WHERE campaign_id IN (...)`. Profiled 16→1 lead-table queries; value identical; contract/cache unchanged; 2 regression tests. (Correctness/security cleared first — backend production-ready on those vectors) | `modules/crm/leads/repository.py`, `modules/marketing/analytics/service.py` |
| Inventory low-stock report N+1 fixed (finding #32, 2026-07-30) — `list_low_stock_items` called `get_stock_level` per active item (3 queries each incl. a redundant lookup + unused avg-cost), i.e. 1+3N; replaced with one grouped `on_hand_by_items` query. Profiled 121→2 queries at 40 items (constant regardless of N); flagged items/quantities identical; fail-before/pass-after test | `modules/inventory/repository.py`, `modules/inventory/service.py` |
| Inventory stock-oversell TOCTOU fixed (finding #27, 2026-07-30) — `issue_stock`/`adjust_stock`/`transfer_stock` checked derived on-hand then inserted with no lock; concurrent reductions could drive stock negative. Now `SELECT ... FOR UPDATE` on the item row serialises reductions. Authenticated data-integrity fix; contract/schema unchanged; 4 regression tests (3 fail pre-fix) | `modules/inventory/service.py`, `modules/inventory/repository.py` |
| Inventory `get_stock_level` `Decimal * float` crash fixed (finding #26, 2026-07-30) — moving-average cost computed `Decimal(quantity) * float(unit_cost)`, raising TypeError (HTTP 500) whenever any receipt existed; broke stock-level view, issue/adjust/transfer, and low-stock report. Normalised to float; 3 regression tests on a previously-uncovered path | `modules/inventory/service.py` |
