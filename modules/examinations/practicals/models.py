...
class PracticalExam(TimestampedBase):
    __tablename__ = "exam_practicals"
...
class PracticalResult(TimestampedBase):
    __tablename__ = "exam_practical_results"
    __table_args__ = (
        UniqueConstraint("practical_id", "student_id", name="uq_practical_result_student"),
    )

    practical_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("exam_practicals.id", ondelete="CASCADE"), nullable=False, index=True
    )
