...
async def create_profile(
    ...
    service = UserProfileService(db)
    profile = await service.create_profile(organization_id, **payload.model_dump())
...
async def get_profile(
    ...
    service = UserProfileService(db)
    profile = await service.get_profile_by_user_id(user_id, organization_id)
    return UserProfilePublic.model_validate(profile)
...
async def update_profile(
    ...
    service = UserProfileService(db)
    profile = await service.update_profile(user_id, organization_id, **payload.model_dump(exclude_unset=True))
    return UserProfilePublic.model_validate(profile)
